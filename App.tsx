...
const App: React.FC = () => {
  const [state, setState] = useState<AppState>(() => {
    const saved = localStorage.getItem('fin_track_state');
      ...
      if (now.getHours() === 22 && now.getMinutes() === 0) { 
        if (Notification.permission === "granted") {
          new Notification("FinTrack Pro", {
            body: "It's 10 PM! Don't forget to log your daily expenses.",
  ...
  }, [hasCompletedOnboarding, notificationsEnabled]);
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white transition-all duration-500 ease-in-out transform lg:translate-x-0 lg:static lg:inset-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full safe-top safe-bottom">
          ...
          <div className="p-4 border-t border-slate-800 animate-in fade-in slide-in-from-bottom-4">
            ...
            <button 
              ...
              className="flex items-center w-full px-4 py-2 text-sm font-medium text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-all duration-300"
            >
        ...
        <header className="bg-white border-b border-slate-200 px-4 pt-3 lg:px-8 z-30 transition-shadow duration-300 shadow-sm safe-top">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-3">
            <div className="flex items-center">
              ...
              <h2 className="text-xl font-bold text-slate-800 capitalize tracking-tight">{activeView}</h2>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-4">
              ...
              <div className="flex gap-2 w-full sm:w-auto">
                <button 
                  ...
                  className="flex-1 sm:flex-none flex items-center justify-center px-5 py-3 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition-all duration-300 shadow-lg shadow-emerald-100 font-bold text-xs whitespace-nowrap active:scale-95"
                >
                ...
                <button 
                  ...
                  className="flex-1 sm:flex-none flex items-center justify-center px-5 py-3 bg-rose-600 text-white rounded-xl hover:bg-rose-700 transition-all duration-300 shadow-lg shadow-rose-100 font-bold text-xs whitespace-nowrap active:scale-95"
                >
  ...
  return (
    <div className="fixed inset-0 bg-slate-50 flex items-center justify-center p-4 z-[100] overflow-hidden">
      <div className="max-w-xl w-full bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden relative">
        ...
        {step === 1 && (
          <div className="p-10 text-center space-y-8 animate-in fade-in slide-in-from-bottom-10 duration-700">
              ...
              <div className="flex items-center bg-white p-4 rounded-2xl border-2 border-slate-100 transition-all hover:border-indigo-100 shadow-sm focus-within:border-indigo-500">
                ...
                <div className="flex-1 text-left">
                  ...
                  <input 
                    ...
                    className="w-full bg-white text-black font-bold outline-none border-b-2 border-transparent focus:border-indigo-500 transition-all py-1 placeholder:text-slate-300"
                  />
                </div>
              </div>
              <button 
                ...
                className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-slate-200 flex items-center justify-center disabled:opacity-50 active:scale-95"
              >
                Continue Setup
        ...
        {step === 2 && (
          <div className="p-10 space-y-8 animate-in fade-in slide-in-from-right-10 duration-700">
            ...
            <div className="space-y-3">
              <PermissionRow 
                ...
                desc="Prevent your browser from automatically clearing data."
                active={permissions.storage}
                onToggle={requestStoragePersistence}
              />
              <PermissionRow 
                icon={Clock} 
                title="Time & Date" 
                desc="Sync with your system clock for accurate transaction dating."
                active={permissions.timedate}
                onToggle={() => setPermissions(p => ({ ...p, timedate: !p.timedate }))}
              />
              <PermissionRow 
                icon={Bell} 
                title="10 PM Reminders" 
                desc="Choose to receive a notification before the day ends."
                active={permissions.notifications}
                onToggle={requestNotifs}
              />
            </div>

            <button 
              onClick={nextStep}
              disabled={!permissions.storage || !permissions.timedate}
              className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100 flex items-center justify-center disabled:opacity-50 active:scale-95"
            >
              Verify Permissions
              <ChevronRight className="w-5 h-5 ml-2" />
            </button>
        ...
        {step === 3 && (
          <div className="p-10 text-center space-y-8 animate-in fade-in slide-in-from-right-10 duration-700">
            ...
            <div className="grid grid-cols-3 gap-3">
              {CURRENCIES.map(c => (
                <button
                  key={c.symbol}
                  onClick={() => setProfile({...profile, currency: c.symbol})}
                  className={`p-5 rounded-2xl border-2 transition-all duration-300 font-black text-xl active:scale-90 ${profile.currency === c.symbol ? 'bg-indigo-600 text-white border-indigo-600 shadow-xl shadow-indigo-100' : 'bg-white text-slate-400 border-slate-100 hover:border-indigo-200 shadow-sm'}`}
                >
                  {c.symbol}
                </button>
              ))}
            ...
            <button 
              onClick={() => onComplete(profile, permissions.notifications)}
              className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-slate-200 active:scale-95"
            >
              Launch Dashboard
            </button>
          </div>
...
const PermissionRow: React.FC<{ 
  ...
  active: boolean, 
  onToggle: () => void 
}> = ({ icon: Icon, title, desc, active, onToggle }) => (
  <button 
    onClick={onToggle}
    className={`w-full flex items-center p-4 rounded-2xl border-2 transition-all duration-500 text-left active:scale-[0.98] ${active ? 'bg-indigo-50 border-indigo-200 shadow-sm' : 'bg-white border-slate-100 hover:border-slate-200'}`}
  >
    <div className={`w-12 h-12 rounded-xl flex items-center justify-center mr-4 transition-colors ${active ? 'bg-indigo-600 text-white' : 'bg-slate-50 text-slate-400'}`}>
    ...
    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${active ? 'bg-indigo-600 border-indigo-600 shadow-sm shadow-indigo-100' : 'border-slate-200'}`}>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6 bg-white">
          <div className="flex p-1.5 bg-slate-100 rounded-2xl">
            <button
              ...
              className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all duration-300 ${type === 'expense' ? 'bg-white text-rose-600 shadow-md' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Expense
            </button>
            <button
              ...
              className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all duration-300 ${type === 'income' ? 'bg-white text-emerald-600 shadow-md' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Income
            </button>
          </div>
          ...
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Value ({currency})</label>
            <input
              type="number"
              required
              autoFocus
              ...
              className="w-full px-6 py-4 bg-white text-black border-2 border-slate-100 rounded-2xl focus:border-indigo-500 focus:ring-0 outline-none transition-all duration-300 text-xl font-black shadow-sm"
              placeholder="0.00"
              step="0.01"
              inputMode="decimal"
            />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full px-5 py-3.5 bg-white text-black border-2 border-slate-100 rounded-2xl focus:border-indigo-500 outline-none transition-all duration-300 text-sm font-bold shadow-sm"
              >
          ...
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Quick Note</label>
            <input
              type="text"
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              className="w-full px-6 py-4 bg-white text-black border-2 border-slate-100 rounded-2xl focus:border-indigo-500 outline-none transition-all duration-300 text-sm font-bold shadow-sm placeholder:text-slate-300"
          ...
          <button
            type="submit"
            className={`w-full py-5 rounded-2xl font-black uppercase tracking-widest text-white transition-all duration-500 shadow-xl transform hover:-translate-y-1 active:scale-95 ${type === 'income' ? 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-200' : 'bg-rose-600 hover:bg-rose-700 shadow-rose-200'}`}
          >
            Confirm Entry
          </button>
        </form>
